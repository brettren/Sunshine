...
public class ForecastAdapter extends CursorAdapter {
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) { //create a new view to hold the data pointed to by cursor
        ...
        int viewType = getItemViewType(cursor.getPosition());  //the current position of the cursor in the row set
        int layoutId = -1;
        switch (viewType) {  // 选择layout的样式
    ...
    public void bindView(View view, Context context, Cursor cursor) { // bind data to view
        ...
        switch (viewType) {
            case VIEW_TYPE_TODAY: {
                ...
                viewHolder.cityNameView.setText(cityName + ", " + locationSetting);
                break;
            }
            case VIEW_TYPE_FUTURE_DAY: {
                // Get weather icon
